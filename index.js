...
if (process.env.ngServerID) MMappID = process.env.ngServerID;
if (process.env.ngChannelID) MMappID = process.env.ngChannelID;
...
client.on("ready", async () => {
  ...
  let ngServer = client.guilds.cache.find(
    (ngGuild) => ngGuild.id === ngServerID
  );
  let announcements = ngServer.channels.cache.find(
    (announc) => announc.id === ngChannelID
  );
...
let ngRef = [
  "Cock joke. username is here",
  "username, just do what comes natural -T",
  "le username has arrived",
  "username, do you remember what a tardigrade is?",
  "Angels sang out in an immaculate chorus, down from the heavends decended username",
  "username was blammed for this post",
  "username has nice titties for a lil boy",
  "Aw gee whiz I hope a username doesn't totally come out of nowhere and own me.",
  "Cryptic metaphor -username",
  "What the hell is private username doing in there?",
];

client.on("guildMemberAdd", async (member) => {
  // code specific to the Flash Holes server
  if (member.guild.id == 283807027720093697) {
    let curRole = member.guild.roles.cache.find(
      (darole) => darole.name === "Flash Hole"
    );
  ...
  if (guildIndex != -1) {
    ...
    let infoPart =
      "*\nYou can use the command `fulpNG` to sign into the Newgrounds API, roles can be added in the <#" +
      ngChannelID +
      "> and `fulpHelp` for more info)";
    ...
    return member.guild.channels.cache
      .find((channel) => channel.id === member.guild.systemChannelID)
      .send("*" + intro + infoPart);
  }
});
...
client.on("message", async (message) => {
  ...
  if (guildIndex != -1) {
    if (
      !message.content.startsWith("[noreact]") &&
      luckyChannels[guildIndex].includes(message.channel.id)
    ) {
      let regShit = new RegExp(
        "((.png|.jpg|.jpeg)|newgrounds.com/(art|audio|portal)/(view|listen))",
        "gi"
      );
      if (message.attachments.size > 0 || regShit.test(message.content)) {
        ...
        message
          .react(
            message.guild.emojis.cache.find((emoji) => emoji.name === "blam")
          ) // + picoSuffix))
          .then((react) =>
            message.react(
              message.guild.emojis.cache.find((emoji) => emoji.name === "1star")
            )
          ) // + picoSuffix)))
          .then((react) =>
            message.react(
              message.guild.emojis.cache.find((emoji) => emoji.name === "2star")
            )
          ) // + picoSuffix)))
          .then((react) =>
            message.react(
              message.guild.emojis.cache.find((emoji) => emoji.name === "3star")
            )
          ) // + picoSuffix)))
          .then((react) =>
            message.react(
              message.guild.emojis.cache.find((emoji) => emoji.name === "4star")
            )
          ) // + picoSuffix)))
          .then((react) =>
            message.react(
              message.guild.emojis.cache.find((emoji) => emoji.name === "5star")
            )
          ); //+ picoSuffix)));
      }
    }
  ...
  if (
    message.content.toLowerCase() === "are we talking about tom fulp?" ||
    message.content.toLowerCase() === "are we talking about tom fulp?"
  ) {
  ...
  if (daServerMetadata != undefined) {
    ...
    if (
      !daServerMetadata.bannedWords.every(function (bannedWord) {
  ...
  if (command == "play" || command == "join") {
    if (!isInGuild) return;
    if (!isDiscordUser) return message.reply(nonDiscordUserMsg);
    ...
    if (!permissions.has("CONNECT") || !permissions.has("SPEAK"))
      return message.channel.send(
        "I can't join or speak with my current permissions in your voice channel :("
      );
    if (
      ...
      return message.channel.send(
        "You do not have permission to speak in this channel, so it is likely you should not be using me either!"
      );

    const searchString = args.slice(0).join(" ");
    const url = args[0] ? args[0].replace(/<(.+)>/g, "$1") : "";

    if (url.startsWith("https://www.newgrounds/audio")) {
      if (url.startsWith("https://www.newgrounds.com/playlists")) {
        const options = {
          uri: url,
        ...
        rp(options).then(($) => {
          ...
          for (let i = 0; i < songList.toArray().length; i++) {
            let daSong = songList.toArray()[i].children[1].children[1].attribs
              .href;
        ...
        handleNGSongs(url, message, message.member.voice.channel);
      }
    } else if (
      url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)
    ) {
      console.log("yo, it's a playlist");
      ...
      try {
        ...
        const song = {
          ...
          onNG: songInfo.videoDetails.video_url.startsWith(
            "https://audio.ngfiles.com"
          ),
        };
        handleVideo(song, message, channel);
      } catch (err) {
        try {
    ...
    if (args[0] > 200)
      return message.channel.send(
        "pls do not use FulpTron for evil (max volume is 200)"
      );
...
**Now playing:** ${serverQueue.songs[0].title}
	...
	*/
    message.channel.send("brandyCringe.png", {
      files: [
        {
    ...
    message.channel.send(curPic, {
      files: [{ attatchment: "pics/delete/" + curPic, name: curPic }],
    });
  }

  if (command == `pic`) {
    if (args[0] == "luis") {
      return message.channel.send("luis.jpg", {
        files: [{ attachment: "pics/luis/" + "luis.jpg", name: "luis.jpg" }],
      });
    }
    ...
    message.channel.send(curPic, {
      files: [{ attachment: `pics/fulp/${curPic}`, name: curPic }],
    });
    ...
    message.channel.send(
      `🎲 You rolled a: ${Math.floor(Math.random() * (max - min + 1)) + min}`
    );
    ...
    if (usr == undefined) {
      ...
      if (usr == "Tom" || usr == "TomFulp") {
        return message.channel.send(
          "Go follow Tom Fulp himself on Newgrounds!\nhttps://TomFulp.newgrounds.com"
        );
      }
      message.channel.send(
        `Go follow ${usr} on Newgrounds!\nhttps://${usr}.newgrounds.com`
      );
    }
    ...
    message.channel
      .send({
        files: [
          "https://cdn.discordapp.com/attachments/422660110830272514/446516094006460417/unknown.png",
        ],
      })
      ...
      .then(
        message.channel.send({
          files: [
            "https://cdn.discordapp.com/attachments/422660110830272514/446516105880535041/unknown.png",
          ],
        })
      );
  } else if (
    ...
    message.channel.send(
      "Dig through my code on Github: \nhttps://github.com/DaemonPlus/fulptronjs"
    ); // TODO this should be more dynamic, for forks and whatnot - Daemon
  }
  ...
  // also check out the cheerio.js github and website
    ...
    rp(options)
      .then(($) => {
        ...
        function infoClean(curList) {
          ...
          listOfShit[listOfShit.length - 1] = listOfShit[
            listOfShit.length - 1
          ...
          for (var i = 0; i < listOfShit.length; i++) {
            ...
            if (dumb.length > 0) secondArray.push(dumb);
          }
      ...
      .catch((err) => {
        console.log(err);
        message.channel.send(
          `an error occured.. did you enter in an actual Newgrounds user??`
        );
      });
    ...
    } else serverInfo = JSON.parse(serverInfo);
    ...
    if (!message.member.hasPermission("MANAGE_MESSAGES")) {
      return message.channel.send(
        "You do not have permissions to add words to the blacklist!"
      );
    }
    ...
    if (isLoggedIn) {
      ...
      request.post(
        ...
        async function (error, response, body) {
          ...
          signedIn =
            !parsedResp.result.data.session.expired &&
            parsedResp.result.data.session.user;
          if (signedIn) {
            ...
            memberInfo.rememberUser =
              parsedResp.result.data.session.rememberUser;
            ...
            if (isDiscordUser) {
              ...
              if (!memberInfo.rememberUser) {
                remember =
                  "In the passport settings, you didn't set 'Remember Me', please check that!";
              }

              message.reply(
                "successfully signed into the Newgrounds API!" + remember
              );
              let role = message.guild.roles.cache.find(
                (darole) => darole.name == "Newgrounder"
              );
              if (role) {
              ...
              message.reply(
                "signed into the Newgrounds API, but without any Discord-specific features."
              );
            }
          }
        }
      ...
      request.post(
        ...
        async function (error, response, body) {
          ...
          } else loggedInUsers = JSON.parse(loggedInUsers);
          ...
          if (isDiscordUser) {
            ...
            console.log(parsedResp);
            message.reply(
              "link sent. Confirm it and then type fulpNG here again!"
            );
            message.author.send(
            ...
            if (args.length != 1) {
              message.reply(
                "looks like you're not a Discord user. To log in, use this same command, but also type your email. (Don't worry, I'll take good care of it!)"
              );
            } else {
              ...
              try {
                ...
                message.reply(
                  "email se--I mean, what's an email? (Psst, type fulpNG here again when you're done. If you get stuck, type fulpNGLogout.)"
                );
              } catch (error) {
                console.log(error);
                if (error.code == "EENVELOPE") {
                  message.reply(
                    "I couldn't send an email to the address you just gave me. Check that it wasn't mistyped."
                  );
                } else if (error.code == "ESOCKET") {
                  message.reply(
                    "I'm unable to send emails right now... Try again later."
                  );
                } else {
                  message.replay(
                    "something bad happened, but I don't know what... Try again later."
                  );
                }
              }
            }
...
function handleNGSongs(songUrl, message, voicechannel, playlist = false) {
  if (!songUrl.startsWith("https://www.newgrounds.com/audio"))
    return message.channel.send(
      `Woops, submission *${songUrl}* is not an audio submission, skipping...`
    );

  songUrl = songUrl.replace("listen", "feed");
  //console.log(songUrl);

  request.get(songUrl, {}, function (error, response, body) {
    let resp = JSON.parse(body);
    //console.log(resp);

    if (!resp.allow_external_api)
      return message.channel.send(
        `Sorry! The author of *${resp.title}* (NG user ${resp.authors[0].name}) has not allowed external API access, so it cannot be played. Message the author if you want this to be changed!`
      );
    ...
    if (song.url.length <= 2)
      return message.channel.send(
        `Song ${song.title} by ${resp.authors[0].name} cannot be played because they are not scouted yet!`
      );
...
async function handleVideo(song, message, voiceChannel, playlist = false) {
  ...
  if (!serverQueue) {
    ...
    try {
      ...
      connection.voice.setSelfDeaf(true); //TODO add self server deaf -Daemon
